...
new_align_call = '''    await nextFrame();\n    // WebKit can expose the new card geometry one paint before the matching\n    // document scroll range. This small settle window keeps first-open motion\n    // deterministic without doing any work during the user's gesture.\n    await new Promise((resolve) => window.setTimeout(resolve, 48));\n    if (token === interactionToken) gentlyAlignOpenContent(name);\n  };\n'''
