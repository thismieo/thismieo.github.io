...
new_align = '''  const retryClampedOpenAlignment = (name, token, desiredTop, userGestureEpoch, finalAttempt = false) => {\n    const delay = finalAttempt ? 120 : 360;\n    window.setTimeout(() => {\n      if (token !== interactionToken || openGroupName !== name || userGestureEpoch !== workshopUserGestureEpoch) return;\n\n      const controls = carouselState[name]?.controls;\n      if (!controls) return;\n      const rect = controls.getBoundingClientRect();\n      if (rect.top <= desiredTop + 36) return;\n\n      const targetY = Math.max(0, window.scrollY + rect.top - desiredTop);\n      const maxY = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);\n      if (maxY < targetY - 2) {\n        if (!finalAttempt) retryClampedOpenAlignment(name, token, desiredTop, userGestureEpoch, true);\n        return;\n      }\n\n      smoothScrollTo(targetY, 220);\n    }, delay);\n  };\n\n  const gentlyAlignOpenContent = (name, token) => {\n    const controls = carouselState[name]?.controls;\n    if (!controls) return;\n    const desiredTop = window.innerWidth <= 700 ? 18 : 28;\n    const rect = controls.getBoundingClientRect();\n    if (Math.abs(rect.top - desiredTop) < 4) return;\n\n    const targetY = Math.max(0, window.scrollY + rect.top - desiredTop);\n    const userGestureEpoch = workshopUserGestureEpoch;\n    smoothScrollTo(targetY);\n    retryClampedOpenAlignment(name, token, desiredTop, userGestureEpoch);\n  };\n'''
