(() => {
  ...
  const warmTouchedSurface = (event) => {
    const surface = resolveSurface(event.target);
    if (surface && !isTactileOnly(surface)) prepareSurface(surface);
  };

  if ("PointerEvent" in window) {
    document.addEventListener("pointerdown", warmTouchedSurface, { capture: true, passive: true });
  } else {
    document.addEventListener("touchstart", warmTouchedSurface, { capture: true, passive: true });
  }

  const releaseLayer = (surface) => {
    if (!surface?.isConnected || surface.classList.contains("is-press-fx")) return;
    findLayer(surface)?.remove();
  };

  const pulse = (surface) => {
    if (!surface?.isConnected) return;
    const layer = prepareSurface(surface);
    const previousTimer = timers.get(surface);
    if (previousTimer) window.clearTimeout(previousTimer);

    surface.classList.remove("is-press-fx");
    const sheen = layer?.querySelector(".press-fx-sheen");
    if (sheen) void sheen.offsetWidth;
    else void surface.offsetWidth;
    surface.classList.add("is-press-fx");

    const timer = window.setTimeout(() => {
      surface.classList.remove("is-press-fx");
      timers.delete(surface);
      if (!isTactileOnly(surface)) requestAnimationFrame(() => releaseLayer(surface));
    }, reduceMotion ? 80 : 600);
  ...
  const clearActiveFeedback = () => {
    ...
    document.querySelectorAll(".press-fx-layer").forEach((layer) => layer.remove());
  };
